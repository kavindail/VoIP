...
void forward_packets(const char *listen_ip, int listen_port, int buffer_size) {
  ...
  std::cout << "Listening for incoming UDP packets on " << listen_ip << ":"
...
int main() {
  ...
  std::thread voiceThread(forward_packets, listenIP, voicePortListen,
                          voiceBufferSize);
  std::thread videoThread(forward_packets, listenIP, videoPortListen,
                          videoBufferSize);
